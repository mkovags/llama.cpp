...
static bool llama_model_load(
    ...
    // load hparams
    {
        ...
        if (hparams.f16 == 4 && n_parts != 1) {
            fprintf(stderr,
                    "%s: GPTQ model detected - are you sure n_parts should be %d? we normally expect it to be 1\n",
                    __func__, n_parts);
            ...
            while (true) {
                ...
                if (progress_callback) {
                    double current_file_progress =
                            double(size_t(fin.tellg()) - file_offset) / double(file_size - file_offset);
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            if (quantize) {
                ...
                switch (type) {
                    case GGML_TYPE_Q4_0: {
                        cur_size = ggml_quantize_q4_0(data_f32.data(), work.data(), nelements, ne[0], qk,
                                                      hist_cur.data());
                    }
                        break;
                    case GGML_TYPE_Q4_1: {
                        cur_size = ggml_quantize_q4_1(data_f32.data(), work.data(), nelements, ne[0], qk,
                                                      hist_cur.data());
                    }
                        break;
                    default: {
...
void llama_print_timings(struct llama_context *ctx) {
    ...
    fprintf(stderr, "%s:      sample time = %8.2f ms / %5d runs   (%8.2f ms per run)\n", __func__,
            1e-3f * ctx->t_sample_us, n_sample, 1e-3f * ctx->t_sample_us / n_sample);
    fprintf(stderr, "%s: prompt eval time = %8.2f ms / %5d tokens (%8.2f ms per token)\n", __func__,
            1e-3f * ctx->t_p_eval_us, n_p_eval, 1e-3f * ctx->t_p_eval_us / n_p_eval);
    fprintf(stderr, "%s:        eval time = %8.2f ms / %5d runs   (%8.2f ms per run)\n", __func__,
            1e-3f * ctx->t_eval_us, n_eval, 1e-3f * ctx->t_eval_us / n_eval);
